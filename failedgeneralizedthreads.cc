...
class Thread {
  ...
  static void* start( Thread* myself, Fn fn, Args... args);
  void action()
  {
	  return;
  }
...
public:
  ...
  template <typename Fn, typename... Args>
  Thread( string name, int priority, Fn fn, Args... args) 
    : name(name), pri(priority)
  {
	...
	pt = thread((void*(*)(void*))start, this, fn, args...);
	//start(this, fn, args...);
  }
...
template <typename Fn, typename... Args>
void* Thread::start(Thread* myself, Fn fn, Args... args) {                     // static.
